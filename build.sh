...
docker run --rm -i -v $PROJECT:/src -v $PROJECT/target/generated:/target --entrypoint atomgraph/saxon xslt -s:/src/structure/source/ubl-advanced-despatch-advice.xml -xsl:/src/tools/remove-pi.xsl -o:/src/rules/examples/AdvancedDespatchAdvice_Example_Full.xml -ext:on --allow-external-functions:on
