...
Below is a list of information elements regarding quantities and units in the format. For all quantities there shall be a legal Unit according to the 
...
Possible limitation to the number of articles that can be ordered. If the Quantity increment is 6 the article shall be ordered in a quantity of 
6, 12, 18 etc.
...
| Consumer unit quantity | Item/PackSizeNumeric | 1 | 6 | 108
...
----
include::../../../../rules/snippets/pre-award-catalogue/Quantities.xml[tags=example1]
----
====
...
----
include::../../../../rules/snippets/pre-award-catalogue/Quantities.xml[tags=example2]
----
====
...
To inform that an item is catch weight (ex. Orderable quantity is pcs, but invoiced quantity is kilo, and  where one pcs can be of variable weight), 
set unit code for content unit to 31 (catch weight) according to UN Recommondations 20. ( <<Unit of measure>> )

[source]
.Example
----
include::../../../../rules/snippets/pre-award-catalogue/Quantities.xml[tags=example3]
----
