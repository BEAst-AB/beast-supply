...
Table 4 shows which information that can be sent, and by repeating the group MeasurementDimension you can send several when using hybrid vehicles for instance. The value in Table 4 is sent in element AttributeID and the value of it in Measure or Description. See example below.
...
|StandardEstimate      |The values have been calculated by using standard consumption for a machine or vehicle. The supplier need to separately inform about how calculation is done.
